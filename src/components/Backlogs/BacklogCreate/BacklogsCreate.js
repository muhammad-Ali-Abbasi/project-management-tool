...
import {setBacklog} from '../../../redux/Actions/BacklogsActions'
...
function getSteps() {
  return ['Select master backlog settings', 'Create an backlog group', 'Description of  backlog'];
}



const BacklogsCreate  = (props) => {
  ...
  const onchangbacklognameHandler=()=>{
    let backlogname = document.getElementById('setbacklogName').value
    setbacklogName(backlogname)
...
const radioArray=[{name:"minor",value:"minor"},{name:"major",value:"major"},{name:"huge",value:"huge"}].map((nature)=>{
    ...
    return (
        ...
        <MuiGrid>
            <Grid item md={3} xs={12}>
                <VerticalStepper steps={steps} activeStep={activeStep} name="backlog" />
            </Grid>
            ...
            <Box>
                ...
                <input style={{width:"80%",border:"1px solid black",outline:"none",padding:"10px",borderRadius:"12px"}} placeholder="backlog Name" onChange={onchangbacklognameHandler}    name="setbacklogName" id="setbacklogName" label="Backlog Name" variant="outlined" fullWidth="100%" ></input>
  ...
  /></div>
                ...
                <Box style={activeStep === steps.length?{display: "block"}:{display:"none"}} >
                  <Typography component="h2" variant="h2" color="primary">Your Backlog is created! </Typography>
                </Box>
              ...
              <Grid item lg={6} sm={12}>
                ...
                <Link to="/BacklogsList"><Button variant="contained" color="primary"  style={activeStep === steps.length ?{display:"block"}:{display:"none"}} > 
                View Backlogs
              </Button></Link>
...
const mapDispatchToProps = dispatch =>({
  setBacklog: Backlog=>dispatch(setBacklog(Backlog))
})

export default connect(null,mapDispatchToProps)(BacklogsCreate) 